...
xdescribe("Takaturn Fund & USDC dummy contract", () => {
    beforeEach(async () => {
        ...
        for (let i = 0; i < totalParticipants; i++) {
            await usdc.methods
                .mint(accounts[i], contributionAmount * 10 ** 8 * totalParticipants)
                .send({
                    from: accounts[15],
                })
        }
    ...
    it("enables participants to pay in USDC and the payments are succesful", async () => {
        ...
        assert.ok(
            (await fund.methods.paidThisCycle(accounts[0]).call()) &&
                (await usdc.methods.balanceOf(fund.options.address).call()) == 1200 * 10 ** 6
        )
    })
})

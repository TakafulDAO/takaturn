...
xdescribe("Collaterals", () => {
    ...
    it("allows users to deposit a collateral and accepts them as participants", async () => {
        ...
        const isCollaterized = await collateral.methods.isCollateralMember(accounts[1]).call()
        assert(isCollaterized)
        const collateralBank = await collateral.methods.collateralMembersBank(accounts[1]).call()
    ...
    it("checks if a user is under collaterized", async () => {
        ...
        const status = await collateral.methods.isUnderCollaterized(accounts[1]).call()
        assert(!status)
    ...
    it("allows users to withdraw collateral in ReleasingCollateral state only", async () => {
        ...
        await withdrawCollateral(accounts[1])
        const balance = await collateral.methods.collateralMembersBank(accounts[1]).call()
        assert(balance == 0)
    })

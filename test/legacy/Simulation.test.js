...
async function makeExcelSheet(outPath) {
    ...
    for (let i = 0; i < totalParticipants; i++) {
        let isDefaulter =
            (await fund.methods.paidThisCycle(participants[i]).call()) &&
            (await fund.methods.currentState().call()) > 2
        ...
        isUnderCollaterized.push(
            await collateral.methods.isUnderCollaterized(participants[i]).call()
        )
        collateralAmounts.push(
            (await collateral.methods.collateralMembersBank(participants[i]).call()) / 10 ** 18
        )
        collateralPayments.push(
            (await collateral.methods.collateralPaymentBank(participants[i]).call()) / 10 ** 18
        )
        balances.push(
            claimedEthBalances[i] +
                (await usdc.methods.balanceOf(participants[i]).call()) * 1 +
                (await fund.methods.beneficiariesPool(participants[i]).call()) * 1 -
    ...
    let csv = []
    csv.push(
        [, , currentCycle, , , chosenBeneficiaryIndex + 1, , , ethPriceInUSD, , , , , , ,].join(";")
    )
    csv.push(
        [, , startingParticipants, , , totalPaidThisCycle, , , , , , , , closeFundReason, ,].join(
            ";"
        )
    )
    csv.push(
    ...
    for (let i = 0; i < totalParticipants; i++) {
        csv.push(
            [
                ...
                /*(collateralPayments[i] + collateralAmounts[i] - 0.044)*/
                ,
                ,
                ,
            ].join(";")
...
async function executeCycle(
    ...
    newEthPrice = 0,
    outPath = path.join(__dirname, "./Simulation_output.csv")
) {
    ...
    if (newEthPrice > 0) {
        await collateral.methods
            .setSimulationEthPrice(web3.utils.toWei(newEthPrice.toString(), "ether"))
            .send({
                from: accounts[12],
            })
    }
    ...
    for (let i = 0; i < totalParticipants; i++) {
        ...
        try {
            ...
            let claimedCollateral =
                (await collateral.methods.collateralPaymentBank(accounts[i]).call()) / 10 ** 18
    ...
    let balanceClaimantOk =
        balanceClaimant - previousBalanceClaimant == contributionAmount * 10 ** 6 * paidAmount
...
xdescribe("Simulations", function () {
    beforeEach(async function () {
        ...
        appendGasLog(
            receipt.to,
            receipt.from,
            "createCollateral(uint totalParticipants, uint cycleTime, uint contributionAmount, uint contributionPeriod, uint collateralAmount)",
            receipt.gasUsed
        )
    ...
    async function randomSimulation(
        outPath,
        ethLower = 800,
        ethHigher = 2000,
        shouldGracefulLimit = false
    ) {
        ...
        for (let i = 0; i < totalParticipants; i++) {
            ...
            let closed = await executeCycle(
                randomInt(parseInt(await fund.methods.totalParticipants().call())),
                [],
                randomInt(ethLower * 100, ethHigher * 100) / 100,
                outPath
            )
    ...
    async function lastBeneficiaryDefaultedSimulation(
        outPath,
        everyonePays = false,
        lowEth = false
    ) {
        ...
        for (let i = 0; i < totalParticipants; i++) {
            ...
            if (!everyonePays) {
                randomDefaultersAmount = randomInt(
                    parseInt(await fund.methods.totalParticipants().call())
                )
            }
            ...
            if (nonBeneficiaries.length == 1) {
                ...
                closed = await executeCycle(
                    randomDefaultersAmount,
                    nonBeneficiaries,
                    randomInt(ethLower * 100, ethHigher * 100) / 100,
                    outPath
                )
    ...
    async function everyoneDefaultedSimulation(
        outPath,
        everyonePays = false,
        lowEth = false,
        defaultAtcycle = totalParticipants
    ) {
        fs.writeFileSync(outPath, "", (err) => {
            if (err) {
                console.error(err)
    ...
    for (let i = 0; i < 10; i++) {
        it("simulation - random " + (i + 1).toString(), async function () {
            ...
            const outPath = path.join(
                __dirname,
                "./simulations/fullRandom/random_" + (i + 1).toString() + ".csv"
            )
    ...
    for (let i = 0; i < 10; i++) {
        it("simulation - random, low eth " + (i + 1).toString(), async function () {
            ...
            const outPath = path.join(
                __dirname,
                "./simulations/fullRandom/randomLowEth_" + (i + 1).toString() + ".csv"
            )
    ...
    for (let i = 0; i < 2; i++) {
        it(
            "simulation - last beneficiary defaulted but everyone pays " + (i + 1).toString(),
            async function () {
                this.timeout(200000)

                const outPath = path.join(
                    __dirname,
                    "./simulations/lastDefaults/lastDefaultsPaid_" + (i + 1).toString() + ".csv"
                )
    ...
    for (let i = 0; i < 10; i++) {
        it(
            "simulation - last beneficiary defaulted, rest is random " + (i + 1).toString(),
            async function () {
                this.timeout(200000)

                const outPath = path.join(
                    __dirname,
                    "./simulations/lastDefaults/lastDefaultsRandom_" + (i + 1).toString() + ".csv"
                )
    ...
    for (let i = 0; i < 10; i++) {
        it(
            "simulation - last beneficiary defaulted, rest is random but eth is low " +
                (i + 1).toString(),
            async function () {
                this.timeout(200000)
                ...
                const outPath = path.join(
                    __dirname,
                    "./simulations/lastDefaults/lastDefaultsRandomLowEth_" +
                        (i + 1).toString() +
                        ".csv"
                )
    ...
    for (let i = 0; i < totalParticipants; i++) {
        it(
            "simulation - one cycle where everyone defaults, but everyone paid before " +
                (i + 1).toString(),
            async function () {
                this.timeout(200000)
                ...
                const outPath = path.join(
                    __dirname,
                    "./simulations/everyoneDefaults/everyoneDefaultsPaid_" +
                        (i + 1).toString() +
                        ".csv"
                )
    ...
    for (let i = 0; i < totalParticipants; i++) {
        it(
            "simulation - one cycle where everyone defaults, rest is random " + (i + 1).toString(),
            async function () {
                this.timeout(200000)

                const outPath = path.join(
                    __dirname,
                    "./simulations/everyoneDefaults/everyoneDefaultsRandom_" +
                        (i + 1).toString() +
                        ".csv"
                )
    ...
    for (let i = 0; i < totalParticipants; i++) {
        it(
            "simulation - one cycle where everyone defaults, everone paid before but eth is low " +
                (i + 1).toString(),
            async function () {
                this.timeout(200000)
                ...
                const outPath = path.join(
                    __dirname,
                    "./simulations/everyoneDefaults/everyoneDefaultsPaidLowEth_" +
                        (i + 1).toString() +
                        ".csv"
                )
    ...
    for (let i = 0; i < totalParticipants; i++) {
        it(
            "simulation - one cycle where everyone defaults, rest is random but eth is low " +
                (i + 1).toString(),
            async function () {
                this.timeout(200000)
                ...
                const outPath = path.join(
                    __dirname,
                    "./simulations/everyoneDefaults/everyoneDefaultsRandomLowEth_" +
                        (i + 1).toString() +
                        ".csv"
                )
    ...
    for (let i = 0; i < 10; i++) {
        it("simulation - cant pay expelled " + (i + 1).toString(), async function () {
            ...
            const outPath = path.join(
                __dirname,
                "./simulations/cantPayExpel/cantPayExpel_" + (i + 1).toString() + ".csv"
            )
    ...
    for (let i = 0; i < 10; i++) {
        it(
            "simulation - random, with a graceful under-collaterized limit" + (i + 1).toString(),
            async function () {
                this.timeout(200000)

                const outPath = path.join(
                    __dirname,
                    "./simulations/gracefulLimit/gracefulLimit_" + (i + 1).toString() + ".csv"
                )
    ...
    for (let i = 0; i < 10; i++) {
        it(
            "simulation - random, low eth, with a graceful under-collaterized limit" +
                (i + 1).toString(),
            async function () {
                this.timeout(200000)
                ...
                const outPath = path.join(
                    __dirname,
                    "./simulations/gracefulLimit/gracefulLimitlowEth_" + (i + 1).toString() + ".csv"
                )

...
async function executeCycle(
    ...
    let fundClosed =
        ...
        parseInt(await fund.methods.currentState().call()) == 5 // FundClosed
    if (fundClosed) {
        assert.ok(true)
    } else {
...
xdescribe("Takaturn Collateral & Fund Part 1", function () {
    ...
    it("can have participants autopay at the end of the funding period", async function () {
        for (let i = 0; i < totalParticipants; i++) {
            await usdc.methods
                .approve(fund.options.address, contributionAmount * 10 ** 6 * totalParticipants)
                .send({
                    from: accounts[i],
                })
    ...
    it("moves multiple defaulters in a row to after the first elligible beneficiary", async function () {
        ...
        for (let i = 0; i < supposedBeneficiaryOrder.length; i++) {
            assert.ok(
                supposedBeneficiaryOrder[i] == (await fund.methods.beneficiariesOrder(i).call())
            )
        }

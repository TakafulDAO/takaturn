...
async function executeCycle(
    ...
    let state = await fund.methods.currentState().call() // todo: este lo agarro del getter
    ...
    if (withdrawFund) {
        for (let i = 0; i < totalParticipants; i++) {
            try {
                ...
                fundClaimed = true // todo: este lo agarro del getter
                break
            } catch (e) {}
        }

        poolEmpty = await fund.methods.beneficiariesPool(claimant).call() // todo: este lo agarro del getter
    }
    ...
    let newCycle = parseInt(await fund.methods.currentCycle().call()) // todo: este lo agarro del getter
    ...
    let fundClosed =
        parseInt(await fund.methods.currentState().call()) == 4 ||
        parseInt(await fund.methods.currentState().call()) == 5 // FundClosed // todo: este lo agarro del getter
    if (fundClosed) {
        assert.ok(true)
    } else {
...
!developmentChains.includes(network.name)
    ...
    : describe("Combined tests", function () {
          xdescribe("Takaturn Collateral & Fund Part 1", function () {
              ...
              it("adds participants", async function () {
                  let lastAddress = await fund.methods
                      .beneficiariesOrder(totalParticipants - 1)
                      .call()
              ...
              it("enables participants to pay in USDC and the payments are succesful", async function () {
                  ...
                  assert.ok(
                      (await fund.methods.paidThisCycle(accounts[0]).call()) &&
                          (await usdc.methods.balanceOf(fund.options.address).call()) ==
                              contributionAmount * 10 ** 6 * totalParticipants
                  )
              })
              ...
              it("moves the order of beneficiaries if the supposed beneficiary of this cycle defaults", async function () {
                  ...
                  let supposedBeneficiaryAfterDefault = await fund.methods
                      .beneficiariesOrder(0)
                      .call()
                  let supposedBeneficiaryNewPosition = await fund.methods
                      .beneficiariesOrder(1)
                      .call()
    ...
    it('makes sure the participants array is empty after the fund is closed', async function() {
        ...
        assert.ok(arrayEmpty);
              ...
              it("returns remaining cycle time properly", async function () {
                  ...
                  assert.ok(
                      currentRemainingCycleTime - newRemainingCycleTime == contributionPeriod + 1
                  )
          ...
          // part 2
          xdescribe("Takaturn Collateral & Fund Part 2", function () {
              beforeEach(async function () {
                  ...
                  ;[collateralAddress] = await factory.methods.getDeployedCollaterals().call()
                  collateral = await new web3.eth.Contract(
                      compiledCollateralSimulation.abi,
                      collateralAddress
                  )
                  ...
                  await collateral.methods
                      .setSimulationEthPrice(web3.utils.toWei(Number(750).toString(), "ether"))
                      .send({
                          from: accounts[12],
                      })
          ...
          // part 3
          xdescribe("Takaturn Collateral & Fund Part 3", function () {
              beforeEach(async function () {
                  totalParticipants = 3
                  cycleTime = 60
                  contributionAmount = 5
          ...
          // part 4
          xdescribe("Takaturn Collateral & Fund Part 4", function () {
              beforeEach(async function () {
                  totalParticipants = 2
                  cycleTime = 60
                  contributionAmount = 5

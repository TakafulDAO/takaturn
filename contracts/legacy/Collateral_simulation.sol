...
contract Collateral_sim is ICollateral, Ownable {
    ...
    function requestContribution(
        ...
        for (uint i = 0; i < defaulters.length; i++) {
            ...
            wasBeneficiary = fundInstance.isBeneficiary(currentDefaulter);
    ...
    function withdrawCollateral() external atState(States.ReleasingCollateral) {
        address sender = msg.sender;
        uint total = collateralMembersBank[sender] + collateralPaymentBank[sender];
    ...
    function isUnderCollaterized(address member) public view returns (bool) {
        ...
        if (fundContract == address(0)) {
            ...
            uint remainingCycles = 1 + counterMembers - currentCycle;
            // Simulation experiment
            if (shouldUseGracefulLimit) {
                // Make under-collaterized limit more graceful by considering historical payments, 0 historical payments = current method
                (uint successfullPayments, , , , ) = fundInstance.getParticipantSummary(member);
                uint performance = (5000 * successfullPayments) / currentCycle;
                collateralLimit =
                    ((remainingCycles * 10000 - performance) * contributionAmount * 10 ** 18) /
                    10000; //convert to Wei
            } else {
                // Use current method
        ...
        memberCollateralUSD = getToUSDConversionRate(int(collateralMembersBank[member]));
    ...
    function emptyCollateralAfterEnd() external onlyOwner atState(States.ReleasingCollateral) {
        require(block.timestamp >= (fundInstance.fundEnd()) + 180 days, "Can't empty yet");
